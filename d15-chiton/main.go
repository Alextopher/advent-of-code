...
func part1(filename string) {
	...
	// Dijkstra's algorithm is the best way to do this
	...
	// print the distance to the bottom right corner
	fmt.Println(dp[size-1][size-1].dist)
}

func part2(filename string) {
	// basically the same as part 1, but we need to massively enlarge the grid
	// Read the input
	file, err := os.Open(filename)
	if err != nil {
		fmt.Println(err)
